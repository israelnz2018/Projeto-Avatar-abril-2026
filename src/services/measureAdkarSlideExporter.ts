...
function getAction(s: any): { text: string; isAuto: boolean } {
  if (s.customActionMeasure && s.customActionMeasure.trim()) {
    return { text: s.customActionMeasure.trim(), isAuto: false };
  }
  const quadrant = getQuadrant(s.power || 'Médio', s.interest || 'Médio');
  const level = levelForPhase(s, 'Measure');
  if (level === 'Verde') {
...
function calcPct(list: any[]): number {
  ...
  const green = list.filter(s => levelForPhase(s, 'Measure') === 'Verde').length;
...
export async function exportMeasureAdkarSlide(
  project: Project,
  toolData: any,
  aiAnalysis: string = '',
  ...
  const slide = createSlide(pres, project,
    'Stakeholders — ADKAR e Plano de Comunicação', 'Measure', aiAnalysis);
  ...
  const drawRow = (s: any) => {
    ...
    const status = s.actionStatusMeasure || 'Pendente';
    const action = getAction(s);
    const cur = s.currentEngagementMeasure ?? '—';
    ...
    // Dots: A (Define) + D (Measure) coloridos, K A R cinza
    const colorA = colorForPhase(s, 'Define');
    const colorD = colorForPhase(s, 'Measure');
    const dotColors = [colorA, colorD, 'D1D5DB', 'D1D5DB', 'D1D5DB'];
  ...
  slide.addText('* D = Desire avaliado na fase Measure. K A R serão avaliados nas fases seguintes.', {
  ...
  const fileName = `Stakeholder_ADKAR_Measure_${sanitize(project.name || 'Projeto')}_${today.replace(/\//g, '')}.pptx`;
