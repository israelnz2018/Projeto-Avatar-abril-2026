...
// Resultado = SOMA das colunas de score (1/3/5). Espelha getRowTotal do componente RABTool.
function rowTotal(row: any, scoreCols: Column[]): number {
  return scoreCols.reduce((sum, c) => sum + (Number(row[c.id]) || 0), 0);
}

function drawRabSlide(
  pres: any,
  project: Project,
  rows: any[],
  pageRows: any[],
  ...
  const slideTitle = totalPages > 1
    ? `Matriz de Priorização RAB (${pageIdx + 1}/${totalPages})`
    : 'Matriz de Priorização RAB';
  const slide = createSlide(pres, project, slideTitle, 'Measure', aiAnalysis);
  ...
  slide.addText('PRIORIZAÇÃO · RAPIDEZ + AUTONOMIA + BENEFÍCIO', {
...
export async function exportRabSlide(
  project: Project,
  toolData: any,
  aiAnalysis: string = '',
  ...
  if (sorted.length === 0) {
    const slide = createSlide(pres, project, 'Matriz de Priorização RAB', 'Measure', aiAnalysis);
    slide.addText('Nenhum item registrado na Matriz RAB.', {
    ...
    pages.forEach((pageRows, idx) => {
      drawRabSlide(pres, project, sorted, pageRows, columns, scoreCols, maxScore, idx, pages.length, aiAnalysis);
    });
  }

  const fileName = `Matriz_RAB_${sanitize(project.name || 'Projeto')}_${today.replace(/\//g, '')}.pptx`;
