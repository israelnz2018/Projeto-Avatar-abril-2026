...
function getAction(s: any): { text: string; isAuto: boolean } {
  if (s.customActionAnalyze && s.customActionAnalyze.trim()) {
    return { text: s.customActionAnalyze.trim(), isAuto: false };
  }
  const quadrant = getQuadrant(s.power || 'Médio', s.interest || 'Médio');
  const level = levelForPhase(s, 'Analyze');
  if (level === 'Verde') {
...
function calcPct(list: any[]): number {
  ...
  const green = list.filter(s => levelForPhase(s, 'Analyze') === 'Verde').length;
...
export async function exportAnalyzeAdkarSlide(
  project: Project,
  toolData: any,
  aiAnalysis: string = '',
  ...
  const slide = createSlide(pres, project,
    'Stakeholders — ADKAR e Plano de Comunicação', 'Analyze', aiAnalysis);
  ...
  const drawRow = (s: any) => {
    ...
    const status = s.actionStatusAnalyze || 'Pendente';
    const action = getAction(s);
    const cur = s.currentEngagementAnalyze ?? '—';
    ...
    // Dots: A (Define) + D (Measure) + K (Analyze) coloridos, A R cinza
    const colorA = colorForPhase(s, 'Define');
    const colorD = colorForPhase(s, 'Measure');
    const colorK = colorForPhase(s, 'Analyze');
    const dotColors = [colorA, colorD, colorK, 'D1D5DB', 'D1D5DB'];
  ...
  slide.addText('* K = Knowledge avaliado na fase Analyze. A R serão avaliados nas fases seguintes.', {
  ...
  const fileName = `Stakeholder_ADKAR_Analyze_${sanitize(project.name || 'Projeto')}_${today.replace(/\//g, '')}.pptx`;
