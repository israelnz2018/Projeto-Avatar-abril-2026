...
function getAction(s: any): { text: string; isAuto: boolean } {
  if (s.customActionControl && s.customActionControl.trim()) {
    return { text: s.customActionControl.trim(), isAuto: false };
  }
  const quadrant = getQuadrant(s.power || 'Médio', s.interest || 'Médio');
  const level = levelForPhase(s, 'Control');
  if (level === 'Verde') {
    return { text: 'Reforço atingido — manter monitoramento de longo prazo.', isAuto: true };
  }
...
function calcPct(list: any[]): number {
  ...
  const green = list.filter(s => levelForPhase(s, 'Control') === 'Verde').length;
...
export async function exportControlAdkarSlide(
  ...
  const drawRow = (s: any) => {
    ...
    const status = s.actionStatusControl || 'Pendente';
    const action = getAction(s);
    const cur = s.currentEngagementControl ?? '—';
    ...
    // Dots: A, D, K, A, R — TODOS coloridos (jornada completa)
    ...
    const colorR  = colorForPhase(s, 'Control');
    const dotColors = [colorA1, colorD, colorK, colorA2, colorR];
  ...
  slide.addText('* Reinforcement avaliado na fase Control. Jornada ADKAR completa.', {
  ...
  const fileName = `Stakeholder_ADKAR_Control_${today.replace(/\//g, '')}.pptx`;
