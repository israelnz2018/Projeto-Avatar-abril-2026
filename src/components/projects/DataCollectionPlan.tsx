...
import { Plus, CheckCircle2, GripVertical, X } from 'lucide-react';
...
export default function DataCollectionPlan({ onSave, initialData }: DataCollectionPlanProps) {
  ...
  const [items, setItems] = useState<DataCollectionItem[]>(initialData?.items || [{ id: '1', data: defaultColumns.reduce((acc, col) => ({ ...acc, [col.id]: '' }), {}) }]);
