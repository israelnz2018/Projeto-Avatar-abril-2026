...
interface StakeholderAdkarProps {
...
const getRecommendedAction = (s: Stakeholder): string => {
  ...
  const awareness = calculateColorForPhase(s, 'Define');
  if (awareness === 'Verde') {
...
export default function StakeholderAdkar({ 
  onSave, 
  initialData, 
  onGenerateAI, 
  isGeneratingAI, 
  ...
  const EXEMPLO_FASES_VISIVEIS = 1; // Define = Awareness
 
  useEffect(() => {
    // If explicitly null, it means it was cleared
    if (initialData === null) {
      setStakeholders([]);
      return;
    }
    ...
    if (initialData?.stakeholders && initialData.stakeholders.length > 0) {
      ...
      const charterStakeholders = charter?.stakeholders || [];
      const charterTeam = charter?.team || [];
      
      // Strict rule: If generated by AI, we must filter out names that don't exist in Charter
      // unless it is an explicitly allowed manual addition.
      // However, usually we just want to ensure we don't have "invented" people.
      let migrated = initialData.stakeholders.map((s: any) => ({
        ...s,
        currentEngagement: s.currentEngagement || 'Neutro',
        desiredEngagement: ROLE_DESIRED_ENGAGEMENT[s.role] || s.desiredEngagement || 'Neutro',
        awareness: s.awareness || 'Vermelho',
        desire: s.desire || 'Cinza',
        knowledge: s.knowledge || 'Cinza',
        ability: s.ability || 'Cinza',
        reinforcement: s.reinforcement || 'Cinza',
        currentEngagementDefine: s.currentEngagementDefine || s.currentEngagement,
        customAction: ''
      }));

      // If it was just generated by AI, enforce the "No invented names" rule strictly
      // and ensure name formatting is correct (e.g. including "a definir")
      if (initialData.isGenerated && (charterStakeholders.length > 0 || charterTeam.length > 0)) {
        const allowedNames = [
          ...charterStakeholders.map((cs: any) => cs.name?.trim()).filter(Boolean),
          ...charterTeam.map((tm: any) => tm.name?.trim()).filter(Boolean)
        ];

        // Filter and fix names
        migrated = migrated.map((s: any) => {
          // Check if this name is an invented one or if it follows the "Role (a definir)" pattern
          const isFromCharter = allowedNames.some(name => s.name.includes(name));
          
          if (!isFromCharter && !s.name.includes('(a definir)')) {
            // It's probably an invented name from AI. Mark it appropriately or skip it?
            // User said "NUNCA gerar stakeholders com nomes inventados".
            // So if it's not a charter name, we skip it if it's from AI.
            return null;
          }
          return s;
        }).filter(Boolean);
    ...
    // IF no saved data, do NOT auto-import anymore. 
    // The user wants it empty until they click "Gerar com IA" or add manually.
        ...
        <div className="hidden md:block text-right">
          ...
          <div className="flex gap-1 mt-1 justify-end">
            {['A'].map((l, i) => {
              ...
              const allGreen = targetList.length > 0 && targetList.every(ld => calculateColorForPhase(ld, 'Define') === 'Verde');
              ...
              return (
                <div key={`${l}-${i}`} className={cn(
                  ...
                  targetList.some(ld => calculateColorForPhase(ld, 'Define') === 'Vermelho') ? "bg-red-400 text-white" :
                  targetList.some(ld => calculateColorForPhase(ld, 'Define') === 'Amarelo') ? "bg-amber-400 text-white" :
            ...
            {isGuideOpen && (
              <div className="p-4 border-t border-gray-200 animate-in slide-in-from-top-2 duration-200">
                <p className="text-xs text-gray-500 mb-4">
                  O Belt avalia o Awareness observando comportamentos reais. Use os critérios abaixo como referência.
                </p>

                <div className="grid grid-cols-1 gap-3 max-w-md">
                  {/* Awareness */}
                  <div className="border border-blue-200 rounded-lg overflow-hidden">
                    <div className="bg-blue-50 px-3 py-2 border-b border-blue-200 text-blue-800">
                      <div className="text-2xl font-black">A</div>
                      <div className="text-[10px] font-bold uppercase">Awareness</div>
                    </div>
                    ...
                    <p className="text-[10px] text-gray-400 italic px-2 pb-2">
                      "A cor é calculada automaticamente comparando o Engajamento Atual da fase Definir com o nível esperado para cada papel."
                    </p>
                  </div>
                </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
          ...
          <div className="flex items-center gap-3">
            {initialData?.isGenerated && onClearAIData && (
              <button onClick={onClearAIData} className="text-[10px] font-bold text-red-500 hover:text-red-700 px-2 py-1 bg-white border border-red-100 rounded">
                Limpar IA
              </button>
            )}
            <button 
              ...
              {isActionsOpen && (
                <div className="p-4 bg-white border-t border-purple-100 mt-1 animate-in slide-in-from-top-2 duration-200">
                  <div className="bg-purple-50 rounded-md p-3 mb-3">
                    <h4 className="font-bold text-sm text-purple-800 mb-1">Foco da fase Definir — Awareness</h4>
                    <p className="text-xs text-purple-700">
                      Fazer cada pessoa SABER que a mudança está acontecendo, por quê e como vai afetá-la.
                    </p>
                  </div>
                    ...
                    <table className="w-full text-left border-collapse">
                      ...
                      <tbody className="text-xs">
                        <tr className="border-b border-gray-100">
                          ...
                          <td className="p-2 text-xs text-gray-700">Conversa individual imediata para explicar o motivo da mudança e o impacto direto. Repetir semanalmente até a pessoa entender.</td>
                        </tr>
                        <tr className="border-b border-gray-100">
                          <td className="p-2"><span className="inline-block px-2 py-0.5 rounded text-[10px] font-bold bg-red-100 text-red-700">Gerenciar de Perto</span></td>
                          <td className="p-2 text-center text-sm">🟡</td>
                          <td className="p-2 text-xs text-gray-700">Conversa individual para reforçar entendimento com dados concretos. Confirmar que ela sabe explicar o motivo da mudança.</td>
                        </tr>
                        <tr className="border-b border-gray-100">
                          ...
                          <td className="p-2 text-center text-sm">🔴</td>
                          <td className="p-2 text-xs text-gray-700">Apresentação executiva com resumo de 1 página explicando o problema, o objetivo e os benefícios esperados.</td>
                        </tr>
                        <tr className="border-b border-gray-100">
                          <td className="p-2"><span className="inline-block px-2 py-0.5 rounded text-[10px] font-bold bg-blue-100 text-blue-700">Manter Satisfeito</span></td>
                          <td className="p-2 text-center text-sm">🟡</td>
                          <td className="p-2 text-xs text-gray-700">Resumo mensal reforçando o motivo e os benefícios. Validar que continua alinhado com o propósito.</td>
                        </tr>
                        <tr className="border-b border-gray-100">
                          ...
                          <td className="p-2 text-center text-sm">🔴</td>
                          <td className="p-2 text-xs text-gray-700">Comunicação oficial sobre o início da mudança. Explicar problema, objetivo e cronograma resumido.</td>
                        </tr>
                        <tr className="border-b border-gray-100">
                          <td className="p-2"><span className="inline-block px-2 py-0.5 rounded text-[10px] font-bold bg-green-100 text-green-700">Manter Informado</span></td>
                          <td className="p-2 text-center text-sm">🟡</td>
                          <td className="p-2 text-xs text-gray-700">Atualização quinzenal reforçando contexto e como a mudança afeta a área dela.</td>
                        </tr>
                        <tr className="border-b border-gray-100">
                          ...
                          <td className="p-2 text-center text-sm">🔴</td>
                          <td className="p-2 text-xs text-gray-700">Mensagem direta com explicação clara da mudança. Disponibilizar canal para tirar dúvidas.</td>
                        </tr>
                        <tr>
                          <td className="p-2"><span className="inline-block px-2 py-0.5 rounded text-[10px] font-bold bg-purple-100 text-purple-700">Monitorar</span></td>
                          <td className="p-2 text-center text-sm">🟡</td>
                          <td className="p-2 text-xs text-gray-700">Lembrete da mudança em comunicações regulares. Reforçar contexto quando houver novidade.</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
            ...
            <table className="w-full text-left border-collapse min-w-[1200px]">
              <thead>
                <tr className="bg-gray-50 text-[10px] font-bold text-gray-500 uppercase tracking-widest border-b border-gray-200">
                  ...
                  <th className="px-4 py-3 text-center w-[120px]">Awareness</th>
          ...
          >
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 sticky top-0 bg-white z-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-black text-gray-800 m-0">Exemplo de ADKAR — Awareness (Consciência)</h3>
