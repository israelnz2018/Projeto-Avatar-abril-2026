...
const ABILITY_ACTIONS: Record<string, Record<string, string>> = {
  'Gerenciar de Perto': {
    'Vermelho': 'Treinamento prático com acompanhamento direto. Coaching individual no local de trabalho até a pessoa executar sem apoio.',
    'Amarelo': 'Acompanhamento da execução com checklist. Feedback semanal sobre o que está funcionando e o que precisa ajustar.',
    'Verde': ''
  },
  'Manter Satisfeito': {
    'Vermelho': 'Demonstração executiva dos resultados do teste prático. Apresentar evidências de que o novo jeito funciona.',
    'Amarelo': 'Resumo com indicadores do teste. Validar aprovação para expandir para mais áreas ou pessoas.',
    'Verde': ''
  },
  'Manter Informado': {
    'Vermelho': 'Treinamento prático em grupo com material de apoio. Acompanhamento por 2 semanas.',
    'Amarelo': 'Sessão de prática supervisionada. Disponibilizar checklist e canal de dúvidas.',
    'Verde': ''
  },
  'Monitorar': {
    'Vermelho': 'Comunicado com os principais instruções claras do novo jeito de trabalhar. Material de consulta acessível.',
    'Amarelo': 'Reforço do novo jeito nas comunicações regulares. Material de referência atualizado.',
    'Verde': ''
  }
};
...
const getRecommendedAction = (s: Stakeholder): string => {
  ...
  const color = calculateColorForPhase(s, 'Improve');
  if (color === 'Verde') {
    return 'Sem ação imediata — manter alinhado nas próximas fases';
  }
  if (color === 'Cinza') return '';
  return ABILITY_ACTIONS[quadrant]?.[color] || '';
};
...
export default function ImproveAdkar({ 
  onSave, 
  initialData, 
  onGenerateAI, 
  isGeneratingAI, 
  ...
  useEffect(() => {
    ...
    if (initialData?.stakeholders && initialData.stakeholders.length > 0) {
      setStakeholders(initialData.stakeholders.map((s: any) => ({
        ...
        currentEngagementImprove: s.currentEngagementImprove || undefined,
        customAction: ''
      })));
      return;
    }
    ...
    if (initialData === undefined) {
      // Senão, puxar da ferramenta AnalyzeAdkar (fase Analyze)
      const previousData = analyzeAdkarData?.toolData || analyzeAdkarData;
      if (previousData?.stakeholders && previousData.stakeholders.length > 0) {
        const imported = previousData.stakeholders.map((s: any) => ({
          ...
          desiredEngagement: s.desiredEngagement || ROLE_DESIRED_ENGAGEMENT[s.role] || 'Neutro',
          currentEngagementDefine: s.currentEngagementDefine || s.currentEngagement,
          currentEngagementImprove: undefined,
          customAction: ''
        }));
        setStakeholders(imported);
      }
    }
  }, [initialData, allProjectData, analyzeAdkarData]);
  ...
  const handleRefreshFromAnalyze = () => {
    const rawSource = analyzeAdkarData || allProjectData?.analyzeAdkar;
    const source = rawSource?.toolData || rawSource;
    if (!source?.stakeholders?.length) {
      alert('Nenhum dado encontrado na ferramenta ADKAR da fase Analisar. Salve os dados lá primeiro.');
      return;
    }
    const imported = source.stakeholders.map((s: any) => ({
      ...
      desiredEngagement: s.desiredEngagement || ROLE_DESIRED_ENGAGEMENT[s.role] || 'Neutro',
      currentEngagementDefine: s.currentEngagementDefine || s.currentEngagement,
      currentEngagementImprove: stakeholders.find(
        ...
        <div className="hidden md:block text-right">
          ...
          <div className="flex gap-1 mt-1 justify-end">
            {['A'].map((l, i) => {
              ...
              const allGreen = targetList.length > 0 && targetList.every(ld => calculateColorForPhase(ld, 'Improve') === 'Verde');
              ...
              return (
                <div key={`${l}-${i}`} className={cn(
                  ...
                  targetList.some(ld => calculateColorForPhase(ld, 'Improve') === 'Vermelho') ? "bg-red-400 text-white" :
                  targetList.some(ld => calculateColorForPhase(ld, 'Improve') === 'Amarelo') ? "bg-amber-400 text-white" :
            ...
            {isGuideOpen && (
              <div className="p-4 border-t border-gray-200 animate-in slide-in-from-top-2 duration-200">
                <p className="text-xs text-gray-500 mb-4">
                  O Belt avalia o Ability observando se a pessoa consegue executar o novo jeito de trabalhar na prática. Use os critérios abaixo como referência.
                </p>
                ...
                <div className="grid grid-cols-1 gap-3 max-w-md">
                  ...
                  <div className="border border-purple-200 rounded-lg overflow-hidden">
                    <div className="bg-purple-50 px-3 py-2 border-b border-purple-200 text-purple-800">
                      ...
                      <div className="text-[10px] font-bold uppercase">ABILITY</div>
                    </div>
                    ...
                    <p className="text-[10px] text-gray-400 italic px-2 pb-2">
                      "A cor é calculada automaticamente comparando o Engajamento Atual da fase Melhorar com o nível esperado para cada papel. Em Melhorar, o foco é fazer cada pessoa CONSEGUIR executar o novo jeito de trabalhar na prática."
                    </p>
                  </div>
                </div>
                    ...
                    <td className="px-4 py-3 align-top">
                      <select 
                        ...
                        onChange={(e) => {
                          ...
                          updateStakeholder(s.id, { currentEngagementImprove: val });
                        }}
                        className={cn(
            ...
            <div className="bg-purple-100/30 border-t border-purple-100 p-2">
              ...
              >
                <div className="flex items-center gap-2 text-purple-700">
                  <Lightbulb size={14} />
                  <span className="text-xs font-medium">Ações Recomendadas — Fase Melhorar (Ability)</span>
                </div>
                <ChevronDown 
                  size={14} 
              ...
              {isActionsOpen && (
                <div className="p-4 bg-white border-t border-purple-100 mt-1 animate-in slide-in-from-top-2 duration-200">
                  <div className="bg-purple-50 rounded-md p-3 mb-3">
                    <h4 className="font-bold text-sm text-purple-800 mb-1">Foco: Ability</h4>
                    <p className="text-xs text-purple-700">
                      Fazer cada pessoa CONSEGUIR executar o novo jeito de trabalhar na prática, com confiança e autonomia.
                    </p>
                  </div>
                    ...
                    <table className="w-full text-left bg-white border-collapse">
                      ...
                      <tbody>
                        <tr className="border-b border-gray-100">
                          ...
                          <td className="p-2 text-xs text-gray-700">Treinamento prático com acompanhamento direto. Coaching individual no local de trabalho até a pessoa executar sem apoio.</td>
                        </tr>
                        <tr className="border-b border-gray-100">
                          <td className="p-2"><span className="inline-block px-2 py-0.5 rounded text-[10px] font-bold bg-red-100 text-red-700">Gerenciar de Perto</span></td>
                          <td className="p-2 text-center text-sm">🟡</td>
                          <td className="p-2 text-xs text-gray-700">Acompanhamento da execução com checklist. Feedback semanal sobre o que está funcionando e o que precisa ajustar.</td>
                        </tr>
                        <tr className="border-b border-gray-100">
                          ...
                          <td className="p-2 text-center text-sm">🔴</td>
                          <td className="p-2 text-xs text-gray-700">Demonstração executiva dos resultados do teste prático. Apresentar evidências de que o novo jeito funciona.</td>
                        </tr>
                        <tr className="border-b border-gray-100">
                          <td className="p-2"><span className="inline-block px-2 py-0.5 rounded text-[10px] font-bold bg-blue-100 text-blue-700">Manter Satisfeito</span></td>
                          <td className="p-2 text-center text-sm">🟡</td>
                          <td className="p-2 text-xs text-gray-700">Resumo com indicadores do teste. Validar aprovação para expandir para mais áreas ou pessoas.</td>
                        </tr>
                        <tr className="border-b border-gray-100">
                          ...
                          <td className="p-2 text-center text-sm">🔴</td>
                          <td className="p-2 text-xs text-gray-700">Treinamento prático em grupo com material de apoio. Acompanhamento por 2 semanas.</td>
                        </tr>
                        <tr className="border-b border-gray-100">
                          <td className="p-2"><span className="inline-block px-2 py-0.5 rounded text-[10px] font-bold bg-green-100 text-green-700">Manter Informado</span></td>
                          <td className="p-2 text-center text-sm">🟡</td>
                          <td className="p-2 text-xs text-gray-700">Sessão de prática supervisionada. Disponibilizar checklist e canal de dúvidas.</td>
                        </tr>
                        <tr className="border-b border-gray-100">
                          ...
                          <td className="p-2 text-center text-sm">🔴</td>
                          <td className="p-2 text-xs text-gray-700">Comunicado com instruções claras do novo jeito de trabalhar. Material de consulta acessível.</td>
                        </tr>
                        <tr>
                          <td className="p-2"><span className="inline-block px-2 py-0.5 rounded text-[10px] font-bold bg-purple-100 text-purple-700">Monitorar</span></td>
                          <td className="p-2 text-center text-sm">🟡</td>
                          <td className="p-2 text-xs text-gray-700">Reforço do novo jeito nas comunicações regulares. Material de referência atualizado.</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
                ...
                <table className="w-full text-left border-collapse min-w-[1200px]">
                  <thead>
                    <tr className="bg-gray-50 text-[10px] font-bold text-gray-500 uppercase tracking-widest border-b border-gray-200">
                      ...
                      <th className="px-4 py-3 text-center w-[120px]">Ability</th>
