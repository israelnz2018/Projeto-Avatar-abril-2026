...
const KNOWLEDGE_ACTIONS: Record<string, Record<string, string>> = {
  'Gerenciar de Perto': {
    'Vermelho': 'Sessão individual para apresentar as descobertas com dados. Tirar todas as dúvidas até ela conseguir explicar a mudança para outra pessoa.',
    'Amarelo': 'Conversa técnica para alinhar os detalhes do "como" será o novo processo. Focar em mitigar riscos operacionais que ela apontar.',
    'Verde': ''
  },
  'Manter Satisfeito': {
    'Vermelho': 'Workshop de demonstração da solução final. Mostrar como o novo processo resolve as dores atuais da área dele.',
    'Amarelo': 'Enviar guia prático ou resumo executivo dos novos procedimentos. Pedir que nomeie "multiplicadores" na equipe.',
    'Verde': ''
  },
  'Manter Informado': {
    'Vermelho': 'Treinamento focado no impacto direto na rotina. Deixar claro o que muda e o que não muda no dia a dia.',
    'Amarelo': 'Pílulas de conhecimento (vídeos curtos ou infográficos). Reforçar onde encontrar os novos materiais de suporte.',
    'Verde': ''
  },
  'Monitorar': {
    'Vermelho': 'Comunicado com os principais conceitos da mudança. Garantir que saiba onde buscar ajuda se precisar interagir com o processo.',
    'Amarelo': 'Informativo padrão sobre o novo fluxo. Disponibilizar FAQ.',
    'Verde': ''
  }
};
...
const getRecommendedAction = (s: Stakeholder): string => {
  ...
  const color = calculateColorForPhase(s, 'Analyze');
  if (color === 'Verde') {
    return 'Sem ação imediata — manter alinhado nas próximas fases';
  }
  if (color === 'Cinza') return '';
  return KNOWLEDGE_ACTIONS[quadrant]?.[color] || '';
};
...
export default function AnalyzeAdkar({ 
  onSave, 
  initialData, 
  onGenerateAI, 
  isGeneratingAI, 
  ...
  const EXEMPLO_FASES_VISIVEIS = 3; // Analyze = Awareness + Desire + Knowledge
 
  useEffect(() => {
    console.log('🔍 AnalyzeAdkar useEffect:', {
      initialData,
      allProjectDataKeys: allProjectData ? Object.keys(allProjectData) : null,
      measureAdkar: measureAdkarData
    });
    ...
    if (initialData?.stakeholders && initialData.stakeholders.length > 0) {
      setStakeholders(initialData.stakeholders.map((s: any) => ({
        ...
        currentEngagementAnalyze: s.currentEngagementAnalyze || undefined,
        customAction: ''
      })));
      return;
    }
    ...
    if (initialData === undefined) {
      // Senão, puxar da ferramenta MeasureAdkar (fase Measure)
      const previousData = measureAdkarData?.toolData || measureAdkarData;
      if (previousData?.stakeholders && previousData.stakeholders.length > 0) {
        const imported = previousData.stakeholders.map((s: any) => ({
          ...
          desiredEngagement: s.desiredEngagement || ROLE_DESIRED_ENGAGEMENT[s.role] || 'Neutro',
          currentEngagementDefine: s.currentEngagementDefine || s.currentEngagement,
          currentEngagementAnalyze: undefined,
          customAction: ''
        }));
        setStakeholders(imported);
      }
    }
  }, [initialData, allProjectData, measureAdkarData]);
  ...
  const handleRefreshFromMeasure = () => {
    const rawSource = measureAdkarData || allProjectData?.measureAdkar;
    const source = rawSource?.toolData || rawSource;
    if (!source?.stakeholders?.length) {
      alert('Nenhum dado encontrado na ferramenta ADKAR da fase Medir. Salve os dados lá primeiro.');
      return;
    }
    const imported = source.stakeholders.map((s: any) => ({
      ...
      desiredEngagement: s.desiredEngagement || ROLE_DESIRED_ENGAGEMENT[s.role] || 'Neutro',
      currentEngagementDefine: s.currentEngagementDefine || s.currentEngagement,
      currentEngagementAnalyze: stakeholders.find(
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-12">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-indigo-700 p-4 rounded-xl shadow-md text-white flex items-center justify-between">
        ...
        <div className="hidden md:block text-right">
          <p className="text-[10px] font-bold uppercase tracking-widest opacity-60">Status Knowledge</p>
          <div className="flex gap-1 mt-1 justify-end">
            {['K'].map((l, i) => {
              const targetList = leaders.length > 0 ? leaders : stakeholders;
              const allGreen = targetList.length > 0 && targetList.every(ld => calculateColorForPhase(ld, 'Analyze') === 'Verde');
              ...
              return (
                <div key={`${l}-${i}`} className={cn(
                  ...
                  targetList.some(ld => calculateColorForPhase(ld, 'Analyze') === 'Vermelho') ? "bg-red-400 text-white" :
                  targetList.some(ld => calculateColorForPhase(ld, 'Analyze') === 'Amarelo') ? "bg-amber-400 text-white" :
            ...
            {isGuideOpen && (
              <div className="p-4 border-t border-gray-200 animate-in slide-in-from-top-2 duration-200">
                <p className="text-xs text-gray-500 mb-4">
                  O Belt avalia o Knowledge observando se a pessoa entende as descobertas e conclusões da análise. Use os critérios abaixo como referência.
                </p>
                ...
                <div className="grid grid-cols-1 gap-3 max-w-md">
                  ...
                  <div className="border border-purple-200 rounded-lg overflow-hidden">
                    <div className="bg-purple-50 px-3 py-2 border-b border-purple-200 text-purple-800">
                      ...
                      <div className="text-[10px] font-bold uppercase">KNOWLEDGE</div>
                    </div>
                    <div className="p-2 space-y-2">
                      ...
                      <div className="flex gap-2">
                        <span className="shrink-0">🟢</span>
                        <p className="text-[10px] text-gray-600 leading-tight">Engajamento Atual está no nível esperado ou acima — pessoa entende as descobertas.</p>
                      </div>
                    </div>
                    <p className="text-[10px] text-gray-400 italic px-2 pb-2">
                      "A cor é calculada automaticamente comparando o Engajamento Atual da fase Analisar com o nível esperado para cada papel. Em Analisar, o foco é fazer cada pessoa ENTENDER as descobertas e conclusões da análise."
                    </p>
                  </div>
                </div>
          ...
          <div className="bg-white border border-purple-100 rounded-lg overflow-hidden shadow-sm mb-4">
            ...
            >
              <div className="flex items-center gap-2 text-purple-700">
                <Lightbulb size={14} />
                <span className="text-xs font-medium">Ações Recomendadas — Fase Analisar (Knowledge)</span>
              </div>
              <ChevronDown 
                size={14} 
            ...
            {isActionsOpen && (
              <div className="p-4 bg-white border-t border-purple-100 mt-1 animate-in slide-in-from-top-2 duration-200">
                <div className="bg-purple-50 rounded-md p-3 mb-3">
                  <h4 className="font-bold text-sm text-purple-800 mb-1">Foco: Knowledge</h4>
                  <p className="text-xs text-purple-700">
                    Fazer cada pessoa ENTENDER as descobertas e conclusões da análise para tomar decisões informadas sobre as próximas ações.
                  </p>
                </div>
                  ...
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="text-xs">
                      <tr className="border-b border-gray-100">
                        ...
                        <td className="p-2 text-xs text-gray-700">Sessão individual para apresentar as descobertas com dados. Tirar todas as dúvidas até ela conseguir explicar a mudança para outra pessoa.</td>
                      </tr>
                      <tr className="border-b border-gray-100">
                        <td className="p-2"><span className="inline-block px-2 py-0.5 rounded text-[10px] font-bold bg-red-100 text-red-700">Gerenciar de Perto</span></td>
                        <td className="p-2 text-center text-sm">🟡</td>
                        <td className="p-2 text-xs text-gray-700">Conversa técnica para alinhar os detalhes do "como" será o novo processo. Focar em mitigar riscos operacionais que ela apontar.</td>
                      </tr>
                      <tr className="border-b border-gray-100">
                        ...
                        <td className="p-2 text-center text-sm">🔴</td>
                        <td className="p-2 text-xs text-gray-700">Workshop de demonstração da solução final. Mostrar como o novo processo resolve as dores atuais da área dele.</td>
                      </tr>
                      <tr className="border-b border-gray-100">
                        <td className="p-2"><span className="inline-block px-2 py-0.5 rounded text-[10px] font-bold bg-blue-100 text-blue-700">Manter Satisfeito</span></td>
                        <td className="p-2 text-center text-sm">🟡</td>
                        <td className="p-2 text-xs text-gray-700">Enviar guia prático ou resumo executivo dos novos procedimentos. Pedir que nomeie "multiplicadores" na equipe.</td>
                      </tr>
                      <tr className="border-b border-gray-100">
                        ...
                        <td className="p-2 text-center text-sm">🔴</td>
                        <td className="p-2 text-xs text-gray-700">Treinamento focado no impacto direto na rotina. Deixar claro o que muda e o que não muda no dia a dia.</td>
                      </tr>
                      <tr className="border-b border-gray-100">
                        <td className="p-2"><span className="inline-block px-2 py-0.5 rounded text-[10px] font-bold bg-green-100 text-green-700">Manter Informado</span></td>
                        <td className="p-2 text-center text-sm">🟡</td>
                        <td className="p-2 text-xs text-gray-700">Pílulas de conhecimento (vídeos curtos ou infográficos). Reforçar onde encontrar os novos materiais de suporte.</td>
                      </tr>
                      <tr className="border-b border-gray-100">
                        ...
                        <td className="p-2 text-center text-sm">🔴</td>
                        <td className="p-2 text-xs text-gray-700">Comunicado com os principais conceitos da mudança. Garantir que saiba onde buscar ajuda se precisar interagir com o processo.</td>
                      </tr>
                      <tr>
                        <td className="p-2"><span className="inline-block px-2 py-0.5 rounded text-[10px] font-bold bg-purple-100 text-purple-700">Monitorar</span></td>
                        <td className="p-2 text-center text-sm">🟡</td>
                        <td className="p-2 text-xs text-gray-700">Informativo padrão sobre o novo fluxo. Disponibilizar FAQ.</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
            ...
            <table className="w-full text-left border-collapse min-w-[1200px]">
              <thead>
                <tr className="bg-gray-50 text-[10px] font-bold text-gray-500 uppercase tracking-widest border-b border-gray-200">
                  ...
                  <th className="px-4 py-3 text-center w-[120px]">Knowledge</th>
          ...
          >
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 sticky top-0 bg-white z-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-black text-gray-800 m-0">Exemplo de ADKAR — Knowledge (Conhecimento)</h3>
