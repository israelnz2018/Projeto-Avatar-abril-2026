...
const LandingEmpresarial = lazy(() => import('./components/LandingEmpresarial'));
...
export default function App() {
  ...
  const ehRotaPublica = (() => {
    ...
    const rotasPublicas = ['/formacao', '/empresarial', '/kit90dias', '/verificar/', '/quem-somos', '/contato', '/pacotes-corporativos', '/termos', '/privacidade'];
  ...
  if (typeof window !== 'undefined') {
    ...
    const rotaReservada = path.startsWith('/formacao') || path.startsWith('/empresarial') || path.startsWith('/kit90dias') || path.startsWith('/trilhagratis') || path.startsWith('/verificar/') || ROTAS_INSTITUCIONAIS.some(r => path.startsWith(r));
  ...
  // Landing B2B "LBW para Empresas" — bypass do login.
  if (typeof window !== 'undefined' && window.location.pathname.startsWith('/empresarial')) {
    return (
      <Suspense fallback={<div className="min-h-screen flex items-center justify-center" style={{ background: '#070A18' }}><div className="w-10 h-10 border-4 border-blue-900 border-t-blue-500 rounded-full animate-spin" /></div>}>
        <LandingEmpresarial />
      </Suspense>
